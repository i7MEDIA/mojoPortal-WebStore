	...
	private void PopulateProductControls()
	{
		...
		if (productGuid != Guid.Empty)
		{
			...
			if (product.FulfillmentType == FulfillmentType.Download)
			{
				...
				var productFile = new ProductFile(productGuid);
		...
		txtShippingAmount.Text = $"{product.ShippingAmount:c}";
		txtQuantityOnHand.Text = Invariant($"{product.QuantityOnHand}");
		txtSoldByQtys.Text = product.SoldByQtys;
		txtSortRank1.Text = Invariant($"{product.SortRank1}");
		txtSortRank2.Text = Invariant($"{product.SortRank2}");
	}
