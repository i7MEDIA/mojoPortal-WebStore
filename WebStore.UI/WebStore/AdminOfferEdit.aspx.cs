...
public partial class AdminOfferEditPage : NonCmsBasePage
{
	...
	private readonly ContentMetaRespository metaRepository = new();
	protected WebStoreConfiguration config = new();
	...
	private void PopulateLabels()
	{
		...
		if (c != null)
		{
			...
			crumbs.AddedCrumbs = $"""
				{crumbs.ItemWrapperTop}
				<a href='{SiteRoot}/WebStore/AdminDashboard.aspx?pageid={pageId.ToInvariantString()}&amp;mid={moduleId.ToInvariantString()}' class='unselectedcrumb'>
					{WebStoreResources.StoreManagerLink}
				</a>
				{crumbs.ItemWrapperBottom}
				{crumbs.Separator}
				{crumbs.ItemWrapperTop}
				<a href='{SiteRoot}/WebStore/AdminOffer.aspx?pageid={pageId.ToInvariantString()}&amp;mid={moduleId.ToInvariantString()}' class='unselectedcrumb'>
					{WebStoreResources.OfferAdministrationLink}
				</a>
				{crumbs.ItemWrapperBottom}
			""";
		}
		...
		lnkProductsAdd.NavigateUrl = $"~/WebStore/OfferProductSelectorDialog.aspx"
			.ToLinkBuilder()
			.PageId(pageId)
			.ModuleId(moduleId)
			.AddParam("offer", offerGuid)
			.ToString();
		...
		litAbstractHeader.Text = string.Format(
			displaySettings.AdminPanelHeadingMarkup,
			WebStoreResources.AbstractLabel,
			WebStoreResources.AbstractLabelTipOffer
		);
		litDescriptionHeader.Text = string.Format(
			displaySettings.AdminPanelHeadingMarkup,
			WebStoreResources.DescriptionLabel,
			WebStoreResources.DescriptionLabelTipOffer
		);
	}
	...
	{
		return $"~/WebStore/AdminOfferEdit.aspx"
			.ToLinkBuilder()
			.PageId(pageId)
			.ModuleId(moduleId)
			.AddParam("offer", offerGuid)
	...
	{
		return $"~/WebStore/AdminOffer.aspx"
			.ToLinkBuilder()
			.PageId(pageId)
			.ModuleId(moduleId)
			.ToString();
